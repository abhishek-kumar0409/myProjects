                ...
                for templateFiles in templateFileList:
                    ...
                    for templateFile, templateContents in templateFiles.items():
                        ...
                        if 'resources' in templateContents:
                            ...
                            logger.debug ("Keys present in the TemplateContent Dicts : {0} ".format( templateContents.keys()))
                            ...
                            for resourceTags, resourceContents in templateContents.get('resources').items():
                                ...
                                logger.debug (
                                "=========Resource Tags -K-E-Y-S-> {0} -V-A-L-U-E-S-> {1} ======= ".format( resourceTags, resourceContents ))
                                #logger.debug (
                               #"============Resource Contents ============\n {0}".format( resourceContents ))
                                ...
                                if resourceContents['type'] == 'OS::Nova::Server':
                                    ...
                                    if 'properties' in resourceContents.keys():
                                        logger.debug ("List of Property Tags : {0} ".format( resourceContents['properties'].keys() ))
                                        logger.debug (
                                        "Image Name ----------> {0} ".format(resourceContents['properties']['image']['get_param']))
                                        logger.debug (
                                        "Networks-----------------> {0} ".format(resourceContents['properties']['networks']))
                                                                    ...
                                                                    for k,v in element.items():
                                                                       ...
                                                                       if 'get_resource' in v.keys():
                                                                            for port_name in v.values():
                                                                                ...
                                                                                if port_name in templateContents.get('resources').keys():
                                                                                     print "----------INside Port--------------------->",
                                                                                         ...
                                                                                         if 'network' == port_property_keys:
                                                                                            ...
                                                                                            for port_keys_1 , port_values_1 in port_property_values.items():
                                                                                                ...
                                                                                                if port_keys_1  == 'get_resource':
                                                                                                    print "Resources..."
                                                                                                    network_list.append("N/A")
                                                                                                    if network_list[index] == "N/A":
                                                                                                         jsonResponse['network'] = "N/A"
                                                                                                    else:
                                                                                                         network_params = templateContents['parameters']\
                                                                                                         [network_list[index]] #['net_01', 'net_02']
                                                                                                    ...
                                                                                                    print "Parameters ......."
                                                                                                    network_list.append(port_values_1)
...
#                                            jsonResponse['keyPair'] = "N/A"
                                        ...
                                        logger.debug ( "\n\n\n********************JSON RESPONSE :: {0}************************* ".format ( jsonResponse ))
